 ...
 public:
   ...
   uint8_t getTouch(uint16_t *x, uint16_t *y, uint16_t threshold = 800);
   ...
   bool isTouching() const { return (digitalRead(TOUCH_IRQ) == LOW); }
  ...
  private:
    ...
    inline void
    begin_touch_read_write() __attribute__((always_inline));
    ...
    uint8_t _irq_pin = TOUCH_IRQ;
    uint8_t _cs_pin = TOUCH_CS;
